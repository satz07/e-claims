...
/// @title ClaimRegistryV3 — multi-submitter ClaimRegistry
/// @notice Owner + authorized submitters can upsert claims.
///         Supports parallel tx submission from multiple wallets.
contract ClaimRegistryV3 {
    ...
    modifier onlyAuthorized() {
        require(msg.sender == owner || authorizedSubmitters[msg.sender], "Not authorized");
        _;
    }
    ...
    event ProviderRegistryUpdated(address indexed previous, address indexed next);
    ...
    function addSubmitter(address submitter) external onlyOwner {
        require(submitter != address(0), "Zero address");
    ...
    event ClaimProviderValidated(uint256 indexed claimNumber, bytes32 indexed providerIdHash, bool authorized);
    event ClaimProviderValidationFailed(bytes32 indexed claimIdHash, bytes32 indexed providerIdHash, uint64 atTime);
    ...
    // -------------------------
    // Claims: write (onlyAuthorized — owner OR submitters)
    // -------------------------
    ...
    function upsertClaim(Claim calldata c) external onlyAuthorized {
        _upsertClaim(c);
    }

    function upsertClaimWithProviderCheck(Claim calldata c, bytes32 providerIdHash) external onlyAuthorized {
    ...
    function upsertClaims(Claim[] calldata cs) external onlyAuthorized {
    ...
    // -------------------------
    // Claims: targeted updates (owner only — admin ops)
    // -------------------------
    ...
    // -------------------------
    // Claims: read (any authorized caller)
    // -------------------------
    function getClaim(uint256 claimNumber) external view onlyAuthorized returns (Claim memory) {
        require(claimExists[claimNumber], "Claim not found");
        return claims[claimNumber];
    }

    function getClaimByClaimIdHash(bytes32 claimIdHash) external view onlyAuthorized returns (Claim memory) {
        require(claimIdMapped[claimIdHash], "ClaimId not found");
    ...
    function existsClaim(uint256 claimNumber) external view onlyAuthorized returns (bool) {
        return claimExists[claimNumber];
    }

    // -------------------------
    // Patients: write (onlyAuthorized)
    // -------------------------
    ...
    function upsertPatient(Patient calldata p) external onlyAuthorized {
        _upsertPatient(p);
    }

    function upsertPatients(Patient[] calldata ps) external onlyAuthorized {
    ...
    // Patients: read
    // -------------------------
    function getPatient(bytes32 crIdHash) external view onlyAuthorized returns (Patient memory) {
        require(patientExists[crIdHash], "Patient not found");
        return patients[crIdHash];
    }

    function existsPatient(bytes32 crIdHash) external view onlyAuthorized returns (bool) {
