...
export function buildClaimStruct(parsed: ParsedFhirBundle, claimNumber: bigint) {
  return {
    ...
    providerNameHash: hashUtf8(parsed.fid),
    providerLevelHash: hashUtf8(parsed.facilityLevel),
    patientNameHash: ZERO_HASH,
    accessCodeHash: ZERO_HASH,
    bundleIdHash: hashUtf8(parsed.bundleId),
    crIdHash: hashUtf8(parsed.crId),
    externalIdHash: ZERO_HASH,
    shaCodeHash: hashUtf8(parsed.schemeCode),
    shaPackageCodeHash: parsed.bundleContentHash,
    claimCodeHash: hashUtf8(parsed.interventionCode),
    ...
    guaranteeIdHash: ZERO_HASH,
    explanationHash: ZERO_HASH,
    rejectionReasonHash: ZERO_HASH,
    status: 0,
    surveillanceStatus: 0,
    colourCodeHash: ZERO_HASH,
    count: 0n,
  };
}
